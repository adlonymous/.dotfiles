# Working with me
- Be direct. No glazing. Never write "You're absolutely right!" or sycophantic openers.
...
- Don't say a task is done until typechecks, linters, and tests pass. If none are configured, say so.
...
- If you notice unrelated dead code or bugs, mention them, don't fix the unprompted.
- Clean up orphans your changes create (unused import, variables). Don't remove pre-existing dead code unless asked
...
- Use Oxlint as the default linter for JavaScript and TypeScript projects when the project does not define its own lint command.
...
- When opening pull requests or merge requests, always use a semantic commit messages as the title.
...
- Use Oxlint as the default linter when a project does not define its own lint command.
- For new TypeScript projects I own, install the anti-slop Oxlint plugin unless there is a specific reason not to.
- For existing projects, do not add anti-slop unless asked or already present.
- For expected fallible workflows in TypeScript projects I own, prefer `better-result` with explicit `Result` returns and `TaggedError` variants over throwing, returning `null`, or widening errors to `unknown`.
- For existing projects, do not add `better-result` unless asked or already present; use the `adopt-better-result` skill for audits or named vertical migrations.
...
- Always use the latest versions of Wrangler and Cloudflare's npm packages.
...
- Use Hono for worker apps and Astro/ViNext for frontends when appropriate.

# Working with Github
- Use `gh` for GitHub repositories.
- When writing a pull request (GitHub), be concise. Explain the problem and the solution succinctly.
...
- Use Cloudflare's Browser Run or Kitesurf - for guidance on when to use what, check out: https://developers.cloudflare.com/browser-run/kitesurf
- For things that require checking network requests, use the Chrome Devtools MCP
...
- Whenever you overcome some kind of obstacle or challenge or learn something that could be generally useful across all sessions, prompt to add a note to the global AGENTS.md file so that future sessions can benefit. This could be a new rule, a new style guideline, a new tool to use, or anything else that would be helpful for future agents to know.
